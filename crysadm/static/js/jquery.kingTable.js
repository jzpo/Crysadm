(function ($) {
    ...
    KingTable.prototype.renderThead = function(){
        ...
        if(!!colNum){
            ...
            for(var i=0;i<colNum;i++){
                $th = $("<th style='"+this.options.styles[i]+"'>").html(this.options.head[i]);
                $th.appendTo($tr);
            }
            $tr.appendTo($thead);
        ...
        if(!!rowNum){
            ...
            for(var i=0;i<len;i++){
                if(tempDaTa[i]){
                    ...
                    // 循环创建列  取得对象中的键
                    for(var j=0;j<this.options.key.length;j++){
                        $td = $("<td style='"+this.options.styles[j]+"'>").html(tempDaTa[i][this.options.key[j]]).appendTo($tr);
                    }
                }
            }
    ...
    KingTable.prototype.pagination = function(tdCell){
        ...
        if(this.options.search){
            //搜索文本
            var oInput = $("<input id='search-txt' type='text' style='line-height: 1.5;' class='inline_form_input'/>");
            $td.append(oInput);
            //搜索btn
            var oInputBtn = $("<input id='search-btn' type='button' style='margin-bottom:0px;' value='"+this.options.lang.gopageButtonSearchText+"' class='btn btn-outline btn-default btn-xs'/>");
            $td.append(oInputBtn);
        }
    }
    //表格分页绑定事件
                        ...
                        .on("click",function(){
                            if( tempThis.options.ascORdesc === "asc"){
                                if($.isFunction(o.compareCallBack)){
                                    //之前是升序 点击后变为降序
                                    tempThis.sortCss();
                                    $(this).removeClass('asc bg').addClass("desc");
                                ...
                                if($.isFunction(o.compareCallBack)){
                                    tempThis.sortCss();
                                    $(this).removeClass('desc bg').addClass("asc");
    ...
    KingTable.prototype.sortCss = function(){
        ...
        if( $.isArray(sortContent) && sortContent.length>0){
            $.each(sortContent,function(i,o){
                if(o.index < colNum){
                    tempThis.element.find("thead th").eq(o.index)
                        .removeClass('desc asc').addClass("bg");
                }
            })
        }
    }
