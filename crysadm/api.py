...
# 提交迅雷链接，返回信息
def api_get(cookies, url, data, verify=False, headers=agent_header, timeout=60):
    address = server_address + url
    try:
        proxies = api_proxies()
        r = requests.get(url=address, data=data, proxies=proxies, verify=verify, headers=headers, cookies=cookies, timeout=timeout)
    except requests.exceptions.RequestException as e:
        return __handle_exception(e=e)
...
# 获取速度状态
#    localRequestParams.put("v", 1);
#    localRequestParams.put("type", paramInt);
...
def get_device_stat(s_type, cookies):
    cookies['origin'] = '4' if len(cookies.get('sessionid')) == 128 else '1'
    body = dict(type=s_type, v='3', appversion=appversion)
    return api_post(url='?r=mine/devices_stat', data=body, cookies=cookies)

# 提交收集水晶请求#和别人不一样
def collect(cookies):
    cookies['origin'] = '4' if len(cookies.get('sessionid')) == 128 else '1'
    return api_post(url='?r=mine/collect', data=None, cookies=cookies)
...
# 获取秘银详情
def api_silver_account(cookies):
    cookies['origin'] = '4' if len(cookies.get('sessionid')) == 128 else '1'
    r = api_get("/index.php?r=kcv3/silveraccount&from=1&version=%s" % appversion, cookies=cookies)
    return r
...
def __handle_exception(e=None, rd='接口故障', r=-12345):
    ...
    if b_err_count is None:
        r_session.setex('api_error_count', 60, '1')
        return dict(r=r, rd=rd)

    err_count = int(b_err_count.decode('utf-8')) + 1
    if err_count > 200:
        r_session.setex('api_error_info', 60, '迅雷矿场API故障中,攻城狮正在赶往事故现场,请耐心等待.')

    err_count_ttl = r_session.ttl('api_error_count')
    if err_count_ttl is None:
        err_count_ttl = 30

    r_session.setex('api_error_count', err_count_ttl + 1, str(err_count))
    return dict(r=r, rd=rd)
